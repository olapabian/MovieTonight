...
public interface QuestionAlgorithmRepository extends JpaRepository<Question,Long> {
    ...
    @Query("select q from Question q where q.type = 'RelaseDate' order by random() limit 1")
    ...
    @Query("select q from Question q where q.type = 'Keyword' order by random() limit :amount" )
    List<Question> getKeywordQuestions(@Param("amount") int amount);
